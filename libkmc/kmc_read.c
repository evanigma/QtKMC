...
/*    pixval = pixval%5; */
/*    pixval += 50; */
/*    printf("Pixel value: %ld\n",pixval); */
/*    printf("Pixel values are: %ld\n",pixval); */
/*  55 is a 7 */
...
//	printf("nframes: %ld\n",nframes);
	...
	if (startframeset && (start_frame < 0) ) {
		printf("Error: start frame(%ld) must be >= 0 \n",start_frame); 
		exit(1);
		}
	if (startframeset && (start_frame >= nframes) ) {
		printf("Error: start frame(%ld) must be <= total number of frames on camera \n",start_frame,nframes); 
		exit(1);
		}
	if (endframeset && (end_frame < 0) ) {
		printf("Error: end frame(%ld) must be >= 0\n",end_frame); 
		exit(1);
		}
	if (endframeset && (end_frame >= nframes) ) {
		printf("Error: end frame(%ld) must be <= total number of frames on camera minus one (%ld)\n",end_frame,nframes-1); 
		exit(1);
		}
	if (nframesset && (n_frames < 0) ) {
		printf("Error: number of frames(%ld) must be >=0\n",n_frames); 
		exit(1);
		}
	if (nframesset && (n_frames > nframes) ) {
		printf("Error: number of frames(%ld) must be <= number of frames on camera(%ld)\n", n_frames, nframes); 
		exit(1);
		}
	...
	if (chronological || ((trigger == 0) || (trigger == 3)) ) {
		...
		if (startframeset && endframeset) {
			if ( end_frame < start_frame ) {
				printf("Error: end frame(%ld) must be >= start frame(%ld)\n", end_frame, start_frame); 
				exit(1);
				}
		...
		if (startframeset && nframesset) {
			if ( (start_frame+n_frames-1) > (nframes-1) ) {
				printf("Error: cannot grab %ld frames starting at %ld. Last frame is %ld.\n", n_frames, start_frame, nframes-1); 
				exit(1);
				}
		...
		if (endframeset && nframesset) {
			if ( (end_frame - n_frames + 1) < 0 ) {
				printf("Error: cannot grab %ld frames ending at %ld. First frame must be >0.\n", n_frames, end_frame); 
				exit(1);
				}
	...
	if ( !chronological && (trigger == 1) ) {
		...
		if (startframeset && endframeset) {
			if ( ((start_frame-cfirst+nframes)%nframes) > ((end_frame-cfirst+nframes)%nframes) ) {
				printf("Error: start frame (%ld) must preceed end frame (%ld) chronologically\n", start_frame, end_frame); 
				exit(1);
				}
		...
		if (startframeset && nframesset) {
			if ((((start_frame-cfirst+nframes)%nframes)+n_frames-1) >= nframes){
				printf("Error: cannot transfer (%ld) frames starting at frame (%ld). (%ld is last frame.)\n", n_frames, start_frame, cfirst-1);
				exit(1);
				}
		...
		if (endframeset && nframesset) {
			if ((((end_frame-cfirst+nframes)%nframes)-n_frames+1) < 0) {
				printf("Error: cannot transfer (%ld) frames ending at frame (%ld)\n", n_frames, end_frame);
				exit(1);
				}
	...
	if ( !chronological && (trigger == 2) ) {
		...
		if (startframeset && endframeset) {
			if ( (end_frame < start_frame) && (end_frame != 0) ) {
				printf("Error: end frame(%ld) must be >= start frame(%ld) or 0(last frame)\n", end_frame, start_frame); 
				exit(1);
				}
			if ( (start_frame == 0) && (end_frame != 0) ) {
				printf("Error: end frame (%ld) is before start frame (%ld), which is the last frame\n",end_frame, start_frame);
				exit(1);
				}
		...
		if (startframeset && nframesset) {
			if ( (start_frame+n_frames-1) > (nframes) ) {
				printf("Error: cannot grab %ld frames starting at %ld. Last two frames are %ld, 0.\n", n_frames, start_frame, nframes-1); 
				exit(1);
				}
			if ( (start_frame == 0) && (n_frames > 1) ) {
				printf("Error: cannot grab %ld frames starting at frame %ld, which is the last frame.\n", n_frames, start_frame);
				exit(1);
				}
			end_frame = start_frame + n_frames - 1;
		...
		if (endframeset && nframesset) {
			if ( ((end_frame - n_frames + 1) < 0) && (end_frame != 0) ) {
				printf("Error: cannot grab %ld frames ending at %ld. First frame must be >1.\n", n_frames, end_frame); 
				exit(1);
				}
...
//	printf("nframes: %ld\n",nframes);
//	printf("n_frames: %ld\n",n_frames);
//	for (i=0; i<n_frames; i++) printf("%ld --> %ld\n",grabframenums[i], saveframenums[i] );
...
int read_multiple_frames()
{
    ...
    for ( i=0; i<n_frames; i++) {
	    ...
	    sprintf(numberstring1,"%ld",saveframenums[i]);
	    ...
	    /* Read image to file */
        printf("Saving frame %ld with file index %ld as %s: %s\r",grabframenums[i],saveframenums[i],imageformat,filename);
        fflush(stdout);
...
{
    dev_fd = kmc_open_device(device_name = device);
    save_frames = 1;
    startframeset = 1;
    endframeset = 1;
    get_device_info();
